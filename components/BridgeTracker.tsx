...
export default function BridgeTracker() {
  ...
  useEffect(() => { const saved = localStorage.getItem("bridge_log"); if (saved) setLog(JSON.parse(saved)); }, []);
  function save() {
    const entry = { date: new Date().toISOString().slice(0, 10), score, note };
    const next = [entry, ...log].slice(0, 30);
    setLog(next); localStorage.setItem("bridge_log", JSON.stringify(next)); setNote("");
  }
  return (
    <div className="space-y-4">
      ...
      <textarea value={note} onChange={(e)=>setNote(e.target.value)} placeholder="Note (optional)" className="w-full border p-2 rounded"/>
      ...
      <div className="pt-4">
        ...
        <ul className="text-sm mt-2 space-y-1">
          {log.slice(0,7).map((e,i)=> (<li key={i} className="border p-2 rounded">{e.date} — {e.score} — {e.note}</li>))}
        </ul>
      </div>
    </div>
  );
