...
export default function DecisionCoach() {
  ...
  function run() {
    ...
    else if (t.includes("conflict")) setA("Pause 10 minutes. Write facts only. Offer one concrete next step. No tone words.");
    else if (t.includes("schedule")) setA("Propose two options with dates. Confirm in calendar. Send one-line summary.");
    else setA("Goal → list 2 options → pick lowest-risk child-first path → confirm in writing.");
  }
