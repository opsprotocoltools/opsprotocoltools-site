...
export default function PhaseMap(){
  ...
  useEffect(()=>{
    ...
    const last = raw.slice(-14);
    const mapped = last.map((e:any,i:number)=>({ day: `${i+1}`, phase: Math.max(1, Math.min(5, Math.round(((e?.score??70)/20)))) }));
    ...
    <div className="h-72">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data}>
          ...
          <XAxis dataKey="day"/><YAxis domain={[1,5]} ticks={[1,2,3,4,5]}/>
          <Tooltip/><Line type="monotone" dataKey="phase"/>
        </LineChart>
      </ResponsiveContainer>
